...
#include "../../../include/registrationUtils.h"
...
int main( int argc, char *argv[] )
{
	...
	int OPTSCALE =TOUT+1; double optimizerScale[Dimension]={1.0, 1.0,1.0}; 
	...
	{
		Vparams.push_back("oprimizers scales n" + tostr(a));
...
//			initialParameters.Fill(0.0);
	...
	OptimizerType::ParametersType finalParameters =
		...
		typedef itk::Image< unsigned short, Dimension >  MovingImageType2;
		typedef itk::ImageFileReader<MovingImageType2>  ReaderType2;
		typedef itk::ImageFileWriter< MovingImageType2>  WriterType2;
		typedef itk::ResampleImageFilter<MovingImageType2,MovingImageType2>    ResampleFilterType2;

		ResampleFilterType2::Pointer resampler2 = ResampleFilterType2::New();
		WriterType2::Pointer      writer2 =  WriterType2::New();
		CastFilterType::Pointer  caster =  CastFilterType::New();
		ReaderType2::Pointer ss= ReaderType2::New();
    	ss->SetFileName(  *argv[S] );
		ss->Update();
...
//		transformedImageWrite(fixedCaster->GetOutput(),movingCaster->GetOutput(),finalTransform,resample,writer,caster,O,nt,argc,argv);
		if (*argv[OVF]!=no){
        transformAndWriteImage<FixedImageType,MovingImageType2,TransformType,ResampleFilterType2,WriterType2>(fixedImageReader->GetOutput(),ss->GetOutput(),finalTransform,resampler2,writer2,*argv[O],nt);
		}
		if (argc>OVF)
				if (*argv[OVF]!=no)
				{
