...
int main( int argc, char *argv[] )
{
  if( argc < 4 )
    ...
    std::cerr << " nl [numberOfGridNodesInOneDimension] boundl upperbnound lowerbound nbin mask";
    std::cerr << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if( argc > 5 )
    {
    numberOfGridNodesInOneDimension = atoi( argv[5] );
    }
  // Software Guide : BeginCodeSnippet
  ...
  metric->SetMaximumNumberOfThreads(50);
  typedef itk::LBFGSBOptimizerv4       OptimizerType;
  ...
  boundSelect.Fill( atoi(argv[6]) );
  upperBound.Fill(atof(argv[7]));
  lowerBound.Fill(atoi(argv[8]));
  ...
  RegistrationType::MetricSamplingPercentageArrayType percentagePerLevel;
  percentagePerLevel.SetSize(numberOfLevels);
  RegistrationType::SmoothingSigmasArrayType smoothingSigmasPerLevel;
  smoothingSigmasPerLevel.SetSize(numberOfLevels);

  for (int a = 0; a <numberOfLevels; a++)
  {
	  smoothingSigmasPerLevel[a] = std::pow(2, (numberOfLevels - a)) / 2.0;
	  percentagePerLevel[a] = 1.0 - (((double)a / (double) numberOfLevels) / 2);
	  shrinkFactorsPerLevel[a] = std::pow(2, numberOfLevels - a);

  };

  registration->SetMetricSamplingPercentagePerLevel(percentagePerLevel);
  ...
  try    {    maskReader->Update();    }  catch( itk::ExceptionObject & err )    {    std::cerr << "ExceptionObject caught !" << std::endl;    std::cerr << err << std::endl;    return EXIT_FAILURE;    }
  ...
  typedef itk::MeshFileWriter<MeshType> WriterMeshType;
  	WriterMeshType::Pointer writer = WriterMeshType::New();
  	writer->SetInput(transform->GetMeOutput());
  	writer->SetFileName(argv[3]);
